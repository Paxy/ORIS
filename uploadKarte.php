...
***/ 
  ...
  $uploaddir = 'karte/'; $dir = opendir($uploaddir); 
  $files = array(); 
...
***/ 
    ...
    if ($_FILES["file"]["size"]>3145728)
      echo 'Slika mora biti JPEG formata do 3 MB velicine! (Preveliki fajl)'; 
    else
    if(!eregi('image/jpeg', $_FILES['attachement']['type'])) { 

      echo 'Slika mora biti JPEG formata do 3 MB velicine! (Fajl nije JPG)'; 
...
//        $_FILES['attachement']['name'] = ceil(count($files)+'1').''.strrchr($_FILES['attachement']['name'], '.'); 
	 ...
	 $_FILES['attachement']['name']=strtoupper(substr($_FILES['attachement']['name'],0,1)).substr($_FILES['attachement']['name'],1);
